...
On the **`fa/cloud-dev-fine-tune` branch**, inference runs on **Alibaba DashScope** (`qwen3.6-flash`) for real-time answers (~2–5s/query). Question text and aggregated results (counts, sums) are sent to the cloud for translation and formatting; raw DB rows never leave your server.
...
```
Web Panel (React)  ←→  Backend (FastAPI)  ←→  DashScope (qwen3.6-flash)
                              ↓
...
| **LLM** | Alibaba DashScope · base `qwen3.6-flash` + fine-tuned `qwen3-14b` (SFT/LoRA, toggle via `USE_FINETUNED_MODEL`) |
...
> **Current status:** Phases 0–3 complete; Phase 4 largely done (meta-tools,
> SQL escape hatch, multi-step planner, model fine-tuning, MCP server — all
> feature-flagged). This branch (`fa/cloud-dev-fine-tune`) uses **DashScope
> cloud inference** (base `qwen3.6-flash` + fine-tuned `qwen3-14b`).
...
- ✅ Seed six functions (`get_order_status`, `count_orders`, `get_revenue`, `count_low_reviews`, `top_products`, `list_orders`) — since grown to **~44 functions** across orders, revenue, reviews, products, sellers, customers, and delivery metrics (see [Function Library](#function-library))
...
### Cloud LLM (DashScope) — `fa/cloud-dev-fine-tune` ✅
- ✅ `backend/model_client/` — native DashScope SDK. Base `qwen3.6-flash`
  (multimodal series → `MultiModalConversation`); fine-tuned `qwen3-14b`
  (text model → `Generation` API). The client picks the API automatically.
- ✅ Tool translation + cloud answer formatting
...
### Phase 4 — Long Tail ✅ (mostly done, feature-flagged)
- ✅ Fenced generated-SQL escape hatch (`sql_escape_enabled`; read-only role,
  LIMIT enforced, allowlisted tables)
- ✅ Meta-tool layer — 7 generic shapes routed to ~44 functions (`meta_tools_enabled`)
- ✅ Multi-step / agentic orchestration (`planner_enabled`, `chain_executor.py`)
- ✅ **Model fine-tuning** — SFT/LoRA on `qwen3-14b`; dataset export +
  augmentation (`export_sft_dataset.py`) + base-vs-finetune eval
  (`eval_finetune.py`). Fine-tune is at parity with base on the in-distribution
  eval (both ~92%); base is already at ceiling.
- ✅ **MCP server** (`backend/mcp_server/`) — exposes tools to Cursor/judges;
  see `docs/mcp-demo.md`
...
- **Alibaba DashScope API key** with access to `qwen3.6-flash` ([Model Studio](https://www.alibabacloud.com/help/en/model-studio/get-api-key))
...
The library now spans **~44 registered functions** (`backend/functions/`,
wired in `all_factories.py`) across orders, revenue/payments, reviews,
products/catalog, sellers, customers, and delivery metrics. Each is a
**pre-written parameterized query**; the LLM only fills typed arguments.
...
all 44 tools to the model, the orchestrator can present **7 generic shapes** —
`count`, `rank`, `sum`, `list`, `breakdown`, `compare`, `lookup` (+ a fenced
`query` for the SQL escape hatch) — parameterized by `entity`/`measure`/
`dimension`, then resolves each to a concrete function. Gated by
`meta_tools_enabled`; this is the interface the fine-tune was trained on.
...
- `backend/` — Python FastAPI application
  ...
  - `model_client/` — DashScope client (`qwen3.6-flash` via MultiModalConversation API)
  - `functions/` — ~44 parameterized query functions + registry (+ `sql_escape.py`)
  - `meta_router.py`, `meta_schemas.py` — 7 generic meta-tool shapes → functions
  - `chain_executor.py`, `planner_*.py` — multi-step / agentic orchestration
  - `mcp_server/` — MCP tool server for Cursor/judges
  - `scripts/export_sft_dataset.py`, `scripts/eval_finetune.py` — fine-tune pipeline
...
```bash
cd backend
pytest tests/test_functions.py -v          # core functions against real data
...
Current: eval pass rate target **≥85%** with `qwen3.6-flash` on DashScope. The harness gate is **85%** — its job is to catch regressions below that floor, not to certify the model.
...
✅ **Cloud LLM via DashScope** — `qwen3.6-flash` for tool translation and answer formatting; DB stays local  
...
- [x] Cloud LLM: DashScope `qwen3.6-flash` (`fa/cloud-dev-fine-tune`)
